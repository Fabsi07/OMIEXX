...
public class InvestCommand {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        if (company.getCapital() < betrag) {
            event.reply("❌ Zu wenig Kapital! Vorhanden: " + OmniexxEmbedBuilder.formatMoney(company.getCapital()))
        ...
        if (roll == 0) {
            ...
            effect = "Revenue +" + OmniexxEmbedBuilder.formatMoney(betrag / 10) + "/Tick";
        }
        ...
        event.reply("💸 **" + OmniexxEmbedBuilder.formatMoney(betrag) + " investiert** → " + effect).queue();
    }
}
