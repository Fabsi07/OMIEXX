...
import dev.omniexx.service.techtree.TechNode;
import dev.omniexx.service.techtree.TechTreeService;
...
public class ResearchCommand {
    ...
    private void handleTree(SlashCommandInteractionEvent event) {
        ...
        EmbedBuilder eb = OmniexxEmbedBuilder.base("🔬 Tech-Tree", OmniexxEmbedBuilder.blue())
        ...
        for (TechNode.Pillar pillar : TechNode.Pillar.values()) {
            ...
            for (TechNode node : nodes) {
                ...
                sb.append(String.format("   `%s`  💰 %s  🔬 %d RP%n",
                        node.getKey(),
                        OmniexxEmbedBuilder.formatMoney(node.getCost()),
                        node.getRpCost()));
                if (!isUnlocked) {
    ...
    private void handlePick(SlashCommandInteractionEvent event) {
        ...
        try {
            ...
            EmbedBuilder eb = OmniexxEmbedBuilder.base("✅ Node freigeschaltet: " + node.getName(), OmniexxEmbedBuilder.green())
                    .addField("Säule",          node.getPillar().getLabel(), true)
                    .addField("Tier",           String.valueOf(node.getTier()), true)
                    .addField("Kosten bezahlt", OmniexxEmbedBuilder.formatMoney(node.getCost()) +
    ...
    private void handleStatus(SlashCommandInteractionEvent event) {
        ...
        EmbedBuilder eb = OmniexxEmbedBuilder.base("🔬 Research Status", OmniexxEmbedBuilder.blue())
