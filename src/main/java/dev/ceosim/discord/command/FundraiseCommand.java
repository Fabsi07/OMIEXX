...
public class FundraiseCommand {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        EmbedBuilder eb = CeoEmbedBuilder.base(result.title, result.color)
                .setDescription(result.description)
                .addField("Bewertung deiner Firma", buildScorecard(company), false);

        if (result.capitalGained > 0) {
            eb.addField("💰 Kapital erhalten", CeoEmbedBuilder.formatMoney(result.capitalGained), true);
        }
    ...
    private FundraiseResult calculateOutcome(Company company) {
        ...
        if (score >= 70) {
            ...
            return FundraiseResult.success(
                    ...
                    CeoEmbedBuilder.formatMoney(capital) + " für 15% Equity.",
                    capital, 15, 5, CeoEmbedBuilder.green()
            );
        } else if (score >= 50) {
            ...
            return FundraiseResult.success(
                    ...
                    CeoEmbedBuilder.formatMoney(capital) + " für 20% Equity.",
                    capital, 20, 2, CeoEmbedBuilder.green()
            );
        } else if (score >= 35) {
            ...
            return FundraiseResult.conditional(
                    ...
                    CeoEmbedBuilder.formatMoney(capital) + " — aber du musst in 3 Ticks " +
    ...
    private String buildScorecard(Company company) {
        return String.format("""
                ...
                CeoEmbedBuilder.formatMoney(company.getRevenuePerTick()),
                company.getActiveEmployees().size(),
                company.getTickCount(),
                CeoEmbedBuilder.formatMoney(company.calculateValuation())
        );
    }
