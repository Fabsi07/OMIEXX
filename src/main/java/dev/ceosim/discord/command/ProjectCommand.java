...
import dev.ceosim.entity.CompanyProject;
import dev.ceosim.repository.CompanyRepository;
import dev.ceosim.service.CooldownService;
import dev.ceosim.service.project.ProjectDefinition;
import dev.ceosim.service.project.ProjectService;
...
public class ProjectCommand {
    ...
    private void handleList(SlashCommandInteractionEvent event) {
        ...
        EmbedBuilder eb = CeoEmbedBuilder.base(
                "📋 Projekte — " + company.getMarket().getDisplayName(), CeoEmbedBuilder.blue());
        ...
        for (ProjectDefinition def : projects) {
            ...
            sb.append(String.format("└ ⏱ %d Ticks  •  💰 %s  •  🎁 %s%n",
                    ...
                    CeoEmbedBuilder.formatMoney(def.getCost()),
                    def.getReward()));
    ...
    private void handleStart(SlashCommandInteractionEvent event) {
        ...
        try {
            ...
            EmbedBuilder eb = CeoEmbedBuilder.base("🚀 Projekt gestartet!", CeoEmbedBuilder.green())
                    .addField("Projekt",   def.getName(), true)
                    .addField("Dauer",     def.getDurationTicks() + " Ticks (~" + (def.getDurationTicks() * 6) + "h)", true)
                    .addField("Kosten",    CeoEmbedBuilder.formatMoney(def.getCost()), true)
    ...
    private void handleStatus(SlashCommandInteractionEvent event) {
        ...
        EmbedBuilder eb = CeoEmbedBuilder.base("🔄 Aktives Projekt", CeoEmbedBuilder.blue())
    ...
    private void handleCancel(SlashCommandInteractionEvent event) {
        ...
        try {
            ...
            event.reply("❌ **Projekt abgebrochen.**\n" +
                    ...
                    CeoEmbedBuilder.formatMoney(active.getCostPaid() / 2) +
