    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        if (attacker.calculateValuation() < MIN_VALUATION) {
            event.reply("🔒 `/sabotage` erst ab **$50k Valuation** verfügbar.\nAktuell: " +
                    CeoEmbedBuilder.formatMoney(attacker.calculateValuation()))
                    .setEphemeral(true).queue();
            return;
        }
        ...
        if (attacker.getCapital() < cost) {
            event.reply("❌ Zu wenig Kapital! Aktion **" + action + "** kostet " +
                    CeoEmbedBuilder.formatMoney(cost) + ".")
                    .setEphemeral(true).queue();
            return;
        }
        ...
        EmbedBuilder eb;
        if (backfire) {
            eb = CeoEmbedBuilder.base("💥 Backfire! Aktion schlug fehl", new Color(0xE74C3C))
                    ...
                    .addField("Kosten",        CeoEmbedBuilder.formatMoney(cost) + " trotzdem verloren", true);
        } else {
            eb = CeoEmbedBuilder.base("🗡️ Sabotage erfolgreich: " + actionLabel(action), new Color(0x8E44AD))
                    .addField("Ziel",          target.getName(), true)
                    .addField("Aktion",        actionLabel(action), true)
                    .addField("Kosten",        CeoEmbedBuilder.formatMoney(cost), true)
    ...
    protected void executeAction(Company attacker, Company target,
        ...
        if (!backfire) {
            eventService.log(attacker, "sabotage_sent",
                    ...
                    "Aktion: " + action + " | Kosten: " + CeoEmbedBuilder.formatMoney(cost),
                    Map.of("capital", -cost));
        }
    }
