...
public class TeamCommand {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        EmbedBuilder eb = CeoEmbedBuilder.base("👥 Team — " + company.getName(), CeoEmbedBuilder.blue());
        ...
        if (employees.isEmpty()) {
            ...
            for (Employee e : employees) {
                sb.append(String.format("%-20s %-14s    %2d      %2d     %s%n",
                        ...
                        e.getSkill(),
                        e.getLoyalty(),
                        CeoEmbedBuilder.formatMoney(e.getSalaryPerTick())
                ));
                totalSalary += e.getSalaryPerTick();
            }
            sb.append("─".repeat(65)).append("\n");
            sb.append(String.format("%-38s Gesamt: %s/Tick%n", "", CeoEmbedBuilder.formatMoney(totalSalary)));
            sb.append("```");
