    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        if (company.getCapital() < salary * 3) { // 3 Ticks Puffer
            event.reply("❌ Zu wenig Kapital für diesen Mitarbeiter! Brauche mindestens **" +
                    CeoEmbedBuilder.formatMoney(salary * 3) + "** als Puffer.")
                    .setEphemeral(true).queue();
            return;
        }
        ...
        EmbedBuilder eb = CeoEmbedBuilder.base("👤 Neuer Mitarbeiter eingestellt!", CeoEmbedBuilder.green())
                ...
                .addField("Gehalt",     CeoEmbedBuilder.formatMoney(emp.getSalaryPerTick()) + "/Tick", true)
